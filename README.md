# MMMEF Sorbonne Courses

This repository hosts course materials and documents I wrote during my MMMEF Master's program at Sorbonne University (Paris).

> **Disclaimer**: This repository contains my personal lecture notes, transcriptions, and additional remarks. It represents my personal version of the courses and is intended for study purposes. It is not an official publication of the university or the professors.

## Course Materials (PDF Access)

Below you will find direct links to the PDF files for the available courses, along with a brief description of their contents.

### Advanced Topics in Machine Learning
- **[Full Course Notes (PDF)](Adv_Tops_in_ML/Adv_Tops_in_ML.pdf)**
  *Covers advanced theoretical and practical aspects of machine learning, including risk minimization, optimization, and learning theory.*

### Arbitrage Theory
- **[Course Notes (French) (PDF)](Arbitrage/francais/ArbitrageTheoryFR.pdf)**
- **[Course Notes (English) (PDF)](Arbitrage/english/ArbitrageTheoryEN.pdf)**
  *Detailed notes on the mathematical theory of arbitrage, asset pricing, and financial markets derivatives.*

### Game Theory
- **[Course Notes (French) (PDF)](GameTheory/francais/GameTheoryFR.pdf)**
- **[Course Notes (English) (PDF)](GameTheory/english/GameTheoryEN.pdf)**
  *Comprehensive coverage of strategic games, Nash equilibria, and cooperative game theory.*
- **[Summary Sheet (French) (PDF)](GameTheory/francais/fiche_synthese.pdf)**
- **[Summary Sheet (English) (PDF)](GameTheory/english/fiche_synthese.pdf)**
  *A concise synthesis of key theorems and definitions for quick revision.*

### Numerical Methods
- **[Full Course Notes (PDF)](Numerical_methods/NumericalMethods.pdf)**
  *Focuses on numerical optimization techniques, algorithms, and their mathematical analysis.*

### Monte Carlo & Malliavin Calculus
- **[Course Notes & Synthesis (PDF)](Malliavin/MalliavinNotes.pdf)**
  *A comprehensive guide covering Monte Carlo foundations, infinite-dimensional analysis (Malliavin Calculus), and applications to sensitivity analysis (Greeks) and hedging.*


# El Karoui Courses

### Quantitative Credit Strategies
- **[Course Notes (French) (PDF)](StratQuantCredit/StratQuantCredit.pdf)**
  *Notes on credit risk modeling, bond pricing, default intensity models, and credit derivatives (CDS).*
