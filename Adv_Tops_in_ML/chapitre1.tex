...
\begin{definition}[Estimateur k-NN]\label{def:knn_regression}
...
\end{align}
Le prédicteur $\hat{f}(x)$ effectue donc une moyenne locale des labels des voisins les plus proches. En classification, cela reviendrait à un vote majoritaire parmi les voisins (voir section Clasification).
\end{exemple}
...
L'approche "plug-in" consiste à considérer des estimateurs $\hat{\pi}_k, \hat{\mu}_k, \hat{\Sigma}_k$ des paramètres inconnus, calculés sur $D_n$, et à les substituer dans l'expression du prédicteur de Bayes (cette approche générale sera formellement détaillée dans la section \ref{sec:plugin_analysis}).
...
\subsection{Analyse Générale du Classifieur Plug-in}\label{sec:plugin_analysis}

Cette sous-section formalise l'approche du "Plug-in Classifier" pour construire un classifieur à partir des données.

\subsubsection{Définition et Approche}
...
\begin{definition}[Classifieur Plug-in]
Le \textbf{Classifieur Plug-in} $\hat{f}$ est défini en remplaçant les probabilités réelles $P_k$ (ou plus généralement tout paramètre inconnu du problème) par des estimateurs $\hat{P}_k$ construits à partir des données :
\[
...
\subsubsection{Borne sur le Risque en Excès}
...
\subsubsection{Analyse Détaillée du Cas Binaire}
...
\begin{definition}[Estimateur des k-Plus Proches Voisins (k-NN) en classification]
L'estimateur k-NN a été défini en régression (voir Théorème \ref{def:knn_regression}). En classification, le principe est similaire.
Soit $D_n = \{(X_i, Y_i)\}_{i=1}^n$ l'échantillon d'apprentissage et $k \ge 1$ un entier.
Pour tout $x \in \R^d$, considérons les données réordonnées selon la distance euclidienne $d(x,X_i) = \|x-X_i\|$, notées $X_{(1)}(x), \dots, X_{(n)}(x)$.
...
\]
Le classifieur des k-NN est alors défini par le principe "Plug-in" (vote majoritaire) :
\[
...
\subsection{Minimisation du Risque Empirique (Empirical Risk Minimization en classification)}
...
\subsubsection{Principe de l'ERM}
...
\subsubsection{Difficultés de l'ERM}
...
\paragraph{Surrogate Loss Convexe}
...
\paragraph{Vecteur de Fonctions de Score}
...
\subsubsection{ERM avec Surrogate $L_2$}
...
\subsubsection{Zhang's Lemma et Consistance}
...
\section{Limites de l'ERM et Transition vers l'Équité}

Tout au long de ce chapitre, nous avons cherché à construire des fonctions de décision $f$ minimisant un risque global $R(f) = \E[L(f(X),Y)]$. L'approche par Minimisation du Risque Empirique (ERM) vise explicitement à maximiser la performance moyenne sur l'ensemble de la population, souvent au détriment des sous-groupes minoritaires. 

En effet, si un groupe représente une faible proportion de l'échantillon d'apprentissage $D_n$, une erreur de prédiction sur ce groupe aura un impact négligeable sur le risque empirique global $\hat{R}(f)$. L'algorithme a donc naturellement tendance à optimiser ses prédictions pour le groupe majoritaire, quitte à être systématiquement inexact pour les minorités. 

De plus, les données $D_n$ peuvent elles-mêmes contenir des biais historiques ou sociaux profonds (ex: discriminations passées à l'embauche ou dans l'octroi de crédits), que l'algorithme se contentera de reproduire ou d'amplifier. La recherche de la seule performance prédictive pure (accuracy) n'est donc pas suffisante dans des applications critiques où les décisions ont un impact humain fort. 

C'est pour répondre à cette limitation fondamentale de l'E.R.M. standard que nous introduisons au chapitre suivant les notions d'\textbf{Équité Algorithmique} (Algorithmic Fairness), afin de contraindre mathématiquement nos modèles à traiter les différents groupes de manière équitable.
