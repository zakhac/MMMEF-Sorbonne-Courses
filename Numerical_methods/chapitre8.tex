...
\begin{tikzpicture}[scale=1.2]
    ...
    % Local solution point - positioned ON THE BOUNDARY to show N not fully in Omega
    \coordinate (xstar) at (2.8, 2.15);
    \filldraw[red] (xstar) circle (2.5pt) node[above left] {$x^*$};
    
    % Neighborhood circle - extends outside Omega
...
\begin{tikzpicture}[scale=1.0]
    % Axes - draw AFTER hatching so they are visible
    % First, create the hatching region
    
    % g1 = 0.1*x^2 + 0.3, g2 = -0.5*x + 3.3
    % Intersection: 0.1*x^2 + 0.5*x - 3 = 0 => x ≈ 3.5
    % At x=3.5: g1 = g2 ≈ 1.525
    
    % Draw hatching only in the region above the max envelope
    % Start hatching from x=0.5 to avoid covering the y-axis
    \begin{scope}
    ...
    % White out below g2 for x < 3.5 (where g2 is max)
    \begin{scope}
    ...
    % White out below g1 for x > 3.5 (where g1 is max)
    \begin{scope}
    ...
    % Label for optimal point - positioned clearly to the right
    \node[red] at (4.8, 1.7) {$(x^*, t^*)$};
    
    % Label for max envelope - positioned to not overlap with g1
    \node at (5.2, 4.7) {$\max_\mu g_\mu(x)$};
    
    % Indicate epigraph with arrow
...
\begin{equation}
    \min_{x \in \mathbb{R}^n} c'x \quad \text{s.c.} \quad Ax = b, \; x \geq 0,
\end{equation}
...
\centering
\begin{tikzpicture}[scale=1.2]
    % Polyhedron (feasible region) in 2D projection
    \coordinate (A) at (0, 0);
    \coordinate (B) at (3, 0);
    ...
    % Vertices (basic feasible solutions)
    ...
    % Optimal vertex - label positioned further right to avoid overlap
    \draw[orange, very thick] (C) circle (5pt);
    \node[orange] at (5.8, 1.5) {\textbf{optimum} ($v_3$)};
    
    % Edge movement arrows
    ...
    \node[green!60!black, right] at (5, 2) {\small Direction de diminution de $c'x$};
\end{tikzpicture}
...
Solve $B'\lambda = c_B$ for $\lambda$\;
Compute $s_N = c_N - N'\lambda$\; \Comment*[r]{Pricing}
\If{$s_N \ge 0$}{
...
Update $x_B^+ = x_B - dx_q^+$, $x_N^+ = (0, \ldots, 0, x_q^+, 0, \ldots, 0)'$\;
...
$W = -I$.
Cône critique $C(P_1) = \ker(\nabla c_1^T) = \{ w \in \mathbb{R}^2 | (2, 2) \cdot w = 0 \} = \{ w | w_1 + w_2 = 0 \}$.
Vecteur du cône : $w = (1, -1)^T$.
Test : $w^T W w = (1, -1) (-I) \begin{pmatrix} 1 \\ -1 \end{pmatrix} = - (1^2 + (-1)^2) = -2 < 0$.
...
Test : $w^T W w = w^T I w = \|w\|^2 > 0$ pour tout $w \ne 0$.
...
$\mathcal{A}(x^*) = \{2\}$.
$C(x^*, \lambda^*) = \{ w \in \mathbb{R}^2 | \nabla c_i^T w = 0 \text{ si } \lambda_i > 0, \text{ et } \nabla c_i^T w \ge 0 \text{ si } \lambda_i = 0 \}$.
Ici $\lambda_2 = 0$ pour contrainte 2 active.
$\nabla c_2 = (0, 1)^T$.
Donc conditions : $\nabla c_2^T w \ge 0 \implies w_2 \ge 0$.
...
Pour tout $w \in C$ non nul : $w^T \begin{pmatrix} 2 & 0 \\ 0 & 2 \end{pmatrix} w = 2(w_1^2 + w_2^2) > 0$.
