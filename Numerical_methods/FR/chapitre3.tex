...
\begin{checkpoint}
\begin{itemize}
    \item \textbf{Directions conjuguées :} Définir deux directions $A$-conjuguées. Quel est l'avantage par rapport à des directions orthogonales simples ?
    \item \textbf{Convergence exacte :} En combien d'itérations au maximum le gradient conjugué converge-t-il pour une fonction quadratique convexe de dimension $n$ ?
    \item \textbf{Krylov :} Quel est le lien entre les directions générées et les espaces de Krylov ?
    \item \textbf{Préconditionnement :} Quel est le but de préconditionner le système $Ax=b$ ? (Réduire le conditionnement $\kappa(A)$ et regrouper les valeurs propres).
\end{itemize}
\end{checkpoint}
