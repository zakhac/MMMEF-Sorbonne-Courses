...
\begin{equation}
    0 > f(x+s) - f(x) \approx \nabla f(x)^\top s \quad \text{(au premier ordre)}.
\end{equation}
C'est-à-dire $\nabla f(x)^\top s < 0$ (pour un pas $s$ suffisamment petit).
...
\begin{equation}
    \begin{cases}
    \nabla c_1(x)^\top d = 0 \quad \text{(tangente à la surface)} \\
    \nabla f(x)^\top d < 0 \quad \text{(descente)}
    \end{cases}
\end{equation}
...
\begin{figureexplanation}
\textbf{Interprétation :} On cherche une direction $d$ telle que $\nabla f^\top d < 0$ (descente) et $\nabla c_1^\top d = 0$ (tangente). Si la droite tangente passe par la zone bleue, une telle direction existe et $x$ n'est pas optimal.
\end{figureexplanation}
...
\begin{enumerate}
    ...
    \begin{equation}
        \nabla f(x)^\top s < 0 \quad \text{et} \quad \nabla c_1(x)^\top s \ge 0.
    \end{equation}
...
\begin{equation}
    \begin{cases}
        \nabla c_i(x^*)^\top d \ge 0 \quad \text{pour } i=1, 2 \quad \text{(réalisable)}, \\
        \nabla f(x^*)^\top d < 0 \quad \text{(descente)}.
    \end{cases}
\end{equation}
...
\begin{definition}[Cône critique]
    ...
    \begin{equation}
        \mathcal{C}(x^*, \lambda^*) = \left\{ w \in \mathcal{F}(x^*) \mid \nabla c_i(x^*)^\top w = 0 \ \text{pour tout } i \in \mathcal{I} \cap \mathcal{A}(x^*) \ \text{avec } \lambda_i^* > 0 \right\}.
    \end{equation}
\end{definition}
...
\begin{remarque}[Équivalence pour le cône critique]
    ...
    \begin{equation}
        ...
        \begin{cases}
            \nabla c_i(x^*)^\top w = 0 & \text{pour tout } i \in \mathcal{E}, \\
            \nabla c_i(x^*)^\top w \geq 0 & \text{pour tout } i \in \mathcal{I} \cap \mathcal{A}(x^*) \text{ avec } \lambda_i^* = 0, \\
            \nabla c_i(x^*)^\top w = 0 & \text{pour tout } i \in \mathcal{I} \cap \mathcal{A}(x^*) \text{ avec } \lambda_i^* > 0.
        \end{cases}
    \end{equation}
    ...
    En effet :
    \begin{itemize}
        \item Pour $i \in \mathcal{E}$, on a $\nabla c_i(x^*)^\top w = 0$ par définition de $\mathcal{F}(x^*)$.
        \item Pour $i \in \mathcal{I} \cap \mathcal{A}(x^*)$ avec $\lambda_i^* > 0$, on a $\nabla c_i(x^*)^\top w = 0$ par définition de $\mathcal{C}(x^*, \lambda^*)$.
...
\end{equation}
où $c(x) = (c_1(x), \dots, c_m(x))^\top$.

\subsection{Fonction Duale et Problème Dual}

\begin{definition}[Fonction objective duale]
    On définit le Lagrangien par $\mathcal{L}(x, \lambda) = f(x) - \lambda^\top c(x)$.
...
\begin{equation}
    (P) \quad \min_{x \in \mathbb{R}^n} c^\top x \quad \text{sujet à} \quad Ax - b \geq 0.
\end{equation}
...
\begin{equation}
    \inf_x (c - A^\top \lambda)^\top x = \begin{cases} 0 & \text{si } c - A^\top \lambda = 0, \\ -\infty & \text{sinon.} \end{cases}
\end{equation}
Donc, pour être dans le domaine $\mathcal{D}$ (où $q > -\infty$), nous devons imposer $c = A^\top \lambda$. Dans ce cas, $q(\lambda) = \lambda^\top b$.

Le problème dual s'écrit alors :
\begin{equation}
    (D) \quad \max_{\lambda} \lambda^\top b \quad \text{sujet à} \quad c = A^\top \lambda, \quad \lambda \geq 0.
\end{equation}
