...
\begin{checkpoint}
\begin{itemize}
    \item \textbf{Vocabulaire :} Saurez-vous définir : fonction objectif, ensemble admissible, solution locale vs globale ?
    \item \textbf{Structure :} Quels sont les ingrédients d'un algorithme de descente (direction + pas) ?
    \item \textbf{Convexité :} Pourquoi est-elle importante ? (Un min local est global).
    \item \textbf{Critères d'arrêt :} Citez deux conditions pour arrêter un algorithme.
    \item \textbf{Hessienne définie positive :} $\nabla^2 f(x^\star)$ est définie positive si $\eta^\top \nabla^2 f(x^\star) \eta > 0$ pour tout vecteur $\eta \neq 0$.
\end{itemize}
\end{checkpoint}
