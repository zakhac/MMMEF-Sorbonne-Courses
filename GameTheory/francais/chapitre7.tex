...
\end{array}
$$
\textit{Explication :} Si P1 joue $W$ et P2 joue $wn'$, le jeu va en $W$ puis en $w$. Gain $(-3,-3)$.
\end{exemple}
...
\begin{definition}[Équilibre Parfait en Sous-Jeu - SPE]
Un profil de stratégies est un \textbf{Équilibre Parfait en Sous-Jeu (SPE)} s'il induit un équilibre de Nash dans \textbf{chaque sous-jeu} de l'arbre (y compris le jeu entier).
\end{definition}

\begin{exemple}[Examen : SPE vs Nash]
Soit le jeu suivant : P1 choisit $A$ ou $D$ (gain 2,2). Si $A$, P2 choisit $l$ ou $r$. Si $l$, P1 choisit $L(4,2)$ ou $R(1,0)$. Si $r$, P1 choisit $L'(0,3)$ ou $R'(3,1)$.
\begin{itemize}
    \item \textbf{Forme Normale :} P1 a 4 stratégies $\{AL, AR, DL, DR\}$. P2 en a 2 $\{l, r\}$.
    \item \textbf{Équilibres de Nash :} $(AL, r)$ (Gain $0,3$), $(DL, l)$ (Gain $2,2$), $(DR, l)$ (Gain $2,2$).
    \item \textbf{Analyse des Sous-Jeux (Induction) :}
    \begin{enumerate}
        \item Sous-jeu après $(A,l)$ : P1 choisit $L$ (4 > 1).
        \item Sous-jeu après $(A,r)$ : P1 choisit $R'$ (3 > 0).
        \item P2 anticipe : s'il joue $l$, P1 jouera $L$ (gain P2 = 2). S'il joue $r$, P1 jouera $R'$ (gain P2 = 1). P2 choisit $l$.
        \item Racine : P1 anticipe que $A$ mène à $L$ (gain 4). $D$ mène à 2. P1 choisit $A$.
    \end{enumerate}
    \item \textbf{Conclusion :} L'unique SPE est le chemin $(A, l, L)$ donné par les stratégies complètes \textbf{(P1: $A, L, R'$; P2: $l$)}.
    \item \textbf{Analyse :} L'équilibre de Nash $(DL, l)$ n'est pas un SPE car il repose sur la menace (implicite) que P1 jouerait mal dans le sous-jeu $(A,r)$ (il ne jouerait pas sa meilleure réponse).
\end{itemize}
\end{exemple}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item J1 anticipe que C mène à $(0, -5)$.
